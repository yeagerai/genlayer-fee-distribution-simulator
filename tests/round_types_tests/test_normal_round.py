...
def test_normal_round(verbose):
    ...
    initialize_constant_stakes(DEFAULT_STAKE, addresses_pool)
    ...
    # Sender Fees Assert
    assert compute_total_costs(
        fee_events,default_budget.senderAddress
    ) == -compute_total_cost(
        default_budget
...
def old_test_normal_round_with_minority_penalties(verbose):
...
def old_test_normal_round_no_majority(verbose):
...
def old_test_normal_round_no_majority_disagree(verbose):
    """Test normal round with no majority (2 Agree, 2 Disagree, 1 Timeout)."""
    # Setup
    rotation = Rotation(
        votes={
