...
from custom_types import TransactionRoundResults, Round, Rotation
...
def pretty_print_fee_distribution(fee_distribution: Dict[str, Dict[str, int]]) -> None:
    ...
    print(f"{Colors.CYAN}Total addresses with fees: {len(active_addresses)} out of {len(fee_distribution)}{Colors.ENDC}\n")
    ...
    # Print header
    header = (f"{Colors.BOLD}{'ADDRESS':<45} | " + 
    ...
    total_by_role = {role: sum(fee_distribution[addr][role] for addr in active_addresses) for role in roles}
    ...
    totals_row = (f"{Colors.BOLD}{'TOTAL':<45} | " + 
                  " | ".join(total_row_parts) + 
                  f" | {Colors.colorize(str(grand_total), Colors.GREEN + Colors.BOLD)}{Colors.ENDC}")
    print(totals_row)


def pretty_print_transaction_results(transaction_results: TransactionRoundResults, round_labels: List[str] = None) -> None:
    """
    ...
    for i, round_obj in enumerate(transaction_results.rounds):
        ...
        if label:
            print(f"{Colors.BOLD}{Colors.CYAN}Round {i}{Colors.ENDC} -- {label_color}{label}{Colors.ENDC}:")
        else:
