...
ROUND_TYPE_PRIORS = {"normal_round": 0.7, "appeal_round": 0.3}
...
def generate_statistical_vote_distribution(
    committee_size: int, consensus_type: str
) -> Dict[str, str]:
    """
    ...
    majority_vote = random.choices(
        vote_options, weights=[VOTE_PRIORS[v] for v in vote_options], k=1
    )[0]
...
def generate_statistical_scenario(
    ...
    for i in range(num_rounds):
        ...
        committee_size = (
            ...
            else random.choice([5, 7, 11, 13, 23])
        )
        ...
        # For appeal rounds, tendency toward stronger consensus
            ...
            consensus_type = random.choices(
                list(CONSENSUS_PRIORS.keys()), weights=consensus_weights, k=1
            )[0]
...
def run_statistical_tests(num_tests: int = 20):
    ...
    print(
        f"{Colors.YELLOW}(Running {num_tests} statistically generated scenarios){Colors.ENDC}"
    )
    ...
    for i in range(single_round_tests):
        transaction_results, transaction_budget = generate_statistical_scenario(
            num_rounds=1
        )
    ...
    for i in range(appeal_tests):
        ...
        run_test(
            i + single_round_tests, transaction_results, transaction_budget, "APPEAL"
        )
