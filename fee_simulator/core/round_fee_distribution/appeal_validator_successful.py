...
def apply_appeal_validator_successful(transaction_results: TransactionRoundResults, round_index: int, budget: TransactionBudget, event_sequence: EventSequence) -> List[FeeEvent]:
    events = []
    ...
    if round.rotations:
        ...
        votes_previous_round = transaction_results.rounds[round_index - 1].rotations[-1].votes
        ...
        if majority == "UNDETERMINED":
            ...
            majority_addresses, minority_addresses = who_is_in_vote_majority(total_votes, majority)
            leader_address = next(iter(total_votes.keys()), None)
