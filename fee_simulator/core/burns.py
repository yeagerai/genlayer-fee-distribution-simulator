...
def compute_unsuccessful_validator_appeal_burn(current_round_index: int, leader_timeout: int, validator_timeout: int, fee_events: List[FeeEvent]) -> float:
    burn = 0
    cost = compute_appeal_bond(current_round_index-1, leader_timeout, validator_timeout)
    earned = 0
    for event in fee_events:
        if event.round_index == current_round_index  and "UNSUCCESSFUL" in event.round_label:
            earned += event.earned
    print(cost, earned)
    burn = cost - earned
    return burn